...
const SEVERITY = {
  BRUTE_FORCE:      { level: "CRITICAL", color: "#ff3355", emoji: "🚨" },
  SQL_INJECTION:    { level: "CRITICAL", color: "#ff3355", emoji: "💉" },
  XSS_ATTACK:       { level: "HIGH",     color: "#f97316", emoji: "⚠️" },
  DDOS_ATTACK:      { level: "CRITICAL", color: "#ff3355", emoji: "💥" },
  CSRF_TEST:        { level: "HIGH",     color: "#f97316", emoji: "🔄" },
  KEYLOGGER:        { level: "HIGH",     color: "#f97316", emoji: "⌨️" },
  FILE_UPLOAD:      { level: "MEDIUM",   color: "#f5a623", emoji: "📁" },
  INVALID_FILE_TYPE:{ level: "MEDIUM",   color: "#f5a623", emoji: "📁" },
  PATH_TRAVERSAL:   { level: "HIGH",     color: "#f97316", emoji: "🗂️" },
  RATE_LIMIT:       { level: "MEDIUM",   color: "#f5a623", emoji: "⏱️" },
  HONEYPOT:         { level: "HIGH",     color: "#f97316", emoji: "🍯" },
  BOT_DETECTED:     { level: "MEDIUM",   color: "#f5a623", emoji: "🤖" },
};

async function sendAttackAlert({ toEmail, userName, attackType, ip, message, timestamp }) {
  const sev = SEVERITY[attackType] || { level: "INFO", color: "#00e8ff", emoji: "🔔" };
  const time = timestamp ? new Date(timestamp).toUTCString() : new Date().toUTCString();
  const subject = `${sev.emoji} [HoneyShield] ${sev.level} — ${attackType.replace(/_/g, " ")} Detected`;
  ...
  <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
    <tr><td style="background:linear-gradient(135deg,#0d1b2e,#0a1525);border:1px solid rgba(0,232,255,0.15);border-radius:12px 12px 0 0;padding:28px 32px;text-align:center;border-bottom:2px solid ${sev.color};">
      <div style="font-size:36px;margin-bottom:8px;">${sev.emoji}</div>
